...
export function parseAvatar(payload: unknown): AvatarPreset | AvatarPhoto | Fail {
  ...
  const claimed = match[1].toLowerCase() === "jpg" ? "jpeg" : match[1].toLowerCase();
  const sniffed = sniffImageMime(buffer);
  if (!sniffed || sniffed !== `image/${claimed}`) {
    return { error: "Upload een echte foto (JPG, PNG of WebP) of kies een look." };
  }
  return { kind: "photo", buffer, mime: sniffed };
}
